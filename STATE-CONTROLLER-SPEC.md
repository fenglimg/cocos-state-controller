...
## 1.7 Capabilities（10 个，CapabilityRegistry 派发）
...
| `tween` | 状态切换补间 | 契约 TBD; ⚠️ 源码引用已注释 enum `EnumPropName.Rotation`（编译风险，见 ⚠️附录B #F-B）|
| `preset` | 预设 save/apply/serialize/deserialize | 契约 TBD(入参/产物/持久化未定义) |
| `codeGen` | 代码生成 generate/getFilename | 契约 TBD; 面板按钮为 TODO mock |

> 验收：capability 由 CapabilityRegistry 注册; 各有 jest **测试清单/CI 证明**（非 Runtime 行为预期本身；TBD 项仅测 registry 注册，不要求完整行为契约）。
...
| 菜单 | 消息 | 预期效果 | 验收点 |
|---|---|---|---|
| 打开/关闭面板 | `open` / `close` | 开/关 state-controller-panel | 面板出现/消失 |
| 探测 inspector | `probe-inspector` | dump inspector DOM 到 inspector-probe.json(调试) | 文件生成 |
| Inspector 增强 开/关 | `inspector-mark-on` / `-off` | 注入/撤销常驻脚本(带持久化 flags) | 选中带 StateSelect 节点见/不见标记 |
...
| state 列表 | 选 / +Add / Del；**Dup 按钮禁用** | 切/增/删 state；复制/移动无面板入口（⏸ 见专项 A）| 选/增/删与 Runtime 同步；Dup 灰显 |
| 录制控件 | 🔴录制 / ⏹停止 / ⤺撤销 | 调 set-recording/cancel-recording；record-badge 反映态 | 录制态切换正确（撤销回滚缺陷见 ⚠️附录B #F-A）|
| 安装核心(sidebar) | 「📥 W3:安装核心」 | **当前为 TODO mock**(仅打印日志) | 点击仅控制台 TODO 日志 |
| **Inspector 增强** 🔨 | 总开关 + 状态可视化/录制脏值/排除徽标 toggle | 总开关→mark-on/off；子开关→inspector-set-flags；`Editor.Profile` 持久化 | 开关即时生效；重启编辑器记忆 |
...
> 审计确认的 7 个代码 bug。文档预期保留正确，缺陷在此列出供修复（TDD 红→绿）。
...
| **#F-B** | MED | TweenCapability 引用已注释 EnumPropName.Rotation（编译风险）| TweenCapability.ts:57/93, StateEnum.ts:158 | — |
...
**专项 A · 触发器交互**
- move/dup/delete **仅 StateController 有**（StateSelect 无）。待定：① StateController 5 触发器是否全可见？② 面板是否补 move/dup 入口（解 F-5）？③ StateSelect 是否加局部 State 变更能力？

**专项 B · UI/UX 注入统一**
- 接 inspector hack 插件方向。待定：① 属性列表注入交互形态（3.7）；② 聚合 ◐ 排除 payload 契约（NA-3）；③ z 可视化（NA-5）；④ 蓝点 definedCount 判据（NA-9）；⑤ 注入 icon / 面板 / 整体 UX 统一规划。
