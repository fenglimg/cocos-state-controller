...
/** 直接在 _ctrlData[ctrlId] 写入指定 state index 对应 stateId 的 propData (值级测试用). */
function seedPage(sel: any, ctrl: any, byStateIndex: { [idx: number]: any }) {
    const ctrlId = ctrl.ctrlId;
    const page = sel._ctrlData[ctrlId] || (sel._ctrlData[ctrlId] = {});
    page.$$stateKeyMode$$ = "stateId";
    for (const k of Object.keys(byStateIndex)) {
        const idx = Number(k);
        const stateId = ctrl.states[idx].stateId;
        page[stateId] = byStateIndex[k as any];
    }
    return page;
}
...
describe("专项A-2 StateSelect 单节点各 state 值 swap/copy/move", () => {
    it("swapStateValues: A1↔B1 值互换, 不动其他 state/节点/selectedIndex/state数量", () => {
        ...
        seedPage(select, ctrl, {
            0: { "cc.Node.opacity": 100 },
            1: { "cc.Node.opacity": 200 },
            2: { "cc.Node.opacity": 255 },
        });
        seedPage(other, ctrl, { 0: { "cc.Node.opacity": 11 }, 1: { "cc.Node.opacity": 22 } });
        ...
        const a = ctrl.states[0].stateId;
        const b = ctrl.states[1].stateId;
        const c = ctrl.states[2].stateId;
        expect(page[a]["cc.Node.opacity"]).toBe(200);
        expect(page[b]["cc.Node.opacity"]).toBe(100);
        // C 槽不动
        expect(page[c]["cc.Node.opacity"]).toBe(255);
        // 其他节点不污染
        expect((other as any)._ctrlData[cid][a]["cc.Node.opacity"]).toBe(11);
        expect((other as any)._ctrlData[cid][b]["cc.Node.opacity"]).toBe(22);
    ...
    it("copyStateValues: A1→B1 深拷覆盖, 源与目标独立 (改一个不影响另一个)", () => {
        ...
        const a = ctrl.states[0].stateId;
        const b = ctrl.states[1].stateId;
        expect(page[b]).toEqual({ "cc.Node.x": 10, "cc.Node.y": 20 });
        // 深拷独立: 改目标不影响源
        page[b]["cc.Node.x"] = -1;
        expect(page[a]["cc.Node.x"]).toBe(10);
        // 源仍在
        expect(page[a]).toEqual({ "cc.Node.x": 10, "cc.Node.y": 20 });
        expect(ctrl.states.length).toBe(3);
    });
    ...
    it("越界 state index 被拒 (返回 false, 不破坏数据)", () => {
        const { ctrl, select } = setup();
        const cid = ctrl.ctrlId;
        seedPage(select, ctrl, { 0: { "cc.Node.opacity": 100 } });
        expect(select.swapStateValues(0, 9, cid)).toBe(false);
        expect(select.copyStateValues(-1, 0, cid)).toBe(false);
        // 数据未动
        expect((select as any)._ctrlData[cid][ctrl.states[0].stateId]["cc.Node.opacity"]).toBe(100);
    });

    it("from===to 为 noop (不丢数据)", () => {
        const { ctrl, select } = setup();
        const cid = ctrl.ctrlId;
        seedPage(select, ctrl, { 1: { "cc.Node.opacity": 123 } });
        expect(select.copyStateValues(1, 1, cid)).toBe(true);
        expect((select as any)._ctrlData[cid][ctrl.states[1].stateId]["cc.Node.opacity"]).toBe(123);
    });
    ...
    describe("inspector 触发器入口 (current↔next)", () => {
        ...
        it("swapValueWithNext: 当前 state 值与下一 state 值互换", () => {
            const { ctrl, select } = setup();
            const cid = ctrl.ctrlId;
            seedPage(select, ctrl, { 0: { "cc.Node.opacity": 100 }, 1: { "cc.Node.opacity": 200 } });
            ctrl.selectedIndex = 0;
            select.swapValueWithNext = true;
            const page = (select as any)._ctrlData[cid];
            expect(page[ctrl.states[0].stateId]["cc.Node.opacity"]).toBe(200);
            expect(page[ctrl.states[1].stateId]["cc.Node.opacity"]).toBe(100);
            expect(ctrl.selectedIndex).toBe(0);
        });
        ...
        it("copyValueToNext: 当前 state 值深拷到下一 state", () => {
            const { ctrl, select } = setup();
            const cid = ctrl.ctrlId;
            seedPage(select, ctrl, { 0: { "cc.Node.x": 7 }, 1: { "cc.Node.x": 0 } });
            ctrl.selectedIndex = 0;
            select.copyValueToNext = true;
            const page = (select as any)._ctrlData[cid];
            expect(page[ctrl.states[1].stateId]).toEqual({ "cc.Node.x": 7 });
        });

        it("最后一个 state 触发 next 操作为安全 noop (无下一 state)", () => {
            const { ctrl, select } = setup();
            const cid = ctrl.ctrlId;
            seedPage(select, ctrl, { 2: { "cc.Node.opacity": 50 } });
            ctrl.selectedIndex = 2; // 最后一个
            select.swapValueWithNext = true; // 无 next, noop
            expect((select as any)._ctrlData[cid][ctrl.states[2].stateId]["cc.Node.opacity"]).toBe(50);
        });
    });
});
