 ...
 *  T9: 同 ctrl 下 sibling StateSelectV2 排除隔离 (per-node _ctrlData)。
...
describe("#T9 sibling StateSelectV2 排除隔离", () => {
    it("ChildA 排除 cc.Node.active 不影响 ChildB", () => {
        const root = new ccL.Node("R9"); const cn = new ccL.Node("C9"); root.addChild(cn);
        const ctrl = cn.addComponent(StateControllerV2); (ctrl as any).__preload();
        const a = new ccL.Node("A9"); cn.addChild(a); const selA = a.addComponent(StateSelectV2); (selA as any).__preload();
        const b = new ccL.Node("B9"); cn.addChild(b); const selB = b.addComponent(StateSelectV2); (selB as any).__preload();
