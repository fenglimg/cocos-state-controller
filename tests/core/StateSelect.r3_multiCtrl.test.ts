 ...
 * T1: isDirectlyControlled 只查父链中间 controller, 不查 targetNode 自身 → 若节点自带 StateController,
...
describe("#T1 isDirectlyControlled 应查 targetNode 自身的 controller", () => {
    it("子节点自带 StateController 时, 祖先 controller 不应直接控制它", () => {
        ...
        const ctrlA = parentNode.addComponent(StateController); (ctrlA as any).__preload();
        const ctrlB = childNode.addComponent(StateController); (ctrlB as any).__preload();
...
describe("#T2 跨 controller 移动后两 controller 缓存应失效重建", () => {
    it("handleControllerTransition 后 旧/新 controller 都 markCacheDirty", () => {
        ...
        const ctrlA = parentA.addComponent(StateController); (ctrlA as any).__preload();
        const ctrlB = parentB.addComponent(StateController); (ctrlB as any).__preload();
        ...
        const select = childNode.addComponent(StateSelect); (select as any).__preload();
