/**
 * brief 必删清单: StateSelectV2 inspector 上的 "工具" 分组 (5 个治标按钮) 应物理删除.
 *
 * 原因: panel 时代的兜底操作 (手动刷新检查器 / 同步内存 / 重新获取控制器等),
 * "架构应自愈" — 现在 inspector 自动同步, 这些按钮失效. UI 入口删, 方法保留作 public API.
 *
 * 契约:
 *   1. StateSelectV2.__props__ 不含 "toolsProps"
 *   2. assets/script/controller/props/StateToolsProps.ts 文件不存在
 *   3. assets/script/controller/StateSelectV2.ts 源码不含 "StateToolsProps" 字符串 (import / type 引用)
 */
...
describe("StateSelectV2 工具组按钮已物理删除 (brief 必删 §6)", () => {
    it("StateSelectV2.__props__ 不含 'toolsProps'", () => {
        // eslint-disable-next-line @typescript-eslint/no-var-requires
        const { StateSelectV2 } = require("../../assets/script/controller/StateSelectV2");
        const props = (StateSelectV2 as any).__props__ as string[];
    ...
    it("StateSelectV2.ts 源码不含 'StateToolsProps' 引用", () => {
